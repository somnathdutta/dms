...
public class UnapproveReasonComposer extends GenericForwardComposer{
	
	Window closewinunapprove;
	...
	public void onClick$btnunapprovereason() throws Exception{
		...
		boolean status = controldocumentDAO.fileUnapproved(googleid);
		...
		if(status){
			...
			closewinunapprove.detach();
			
			
		}else{
			
			alert("Document could not be unapproved");
		}
